...
class TestPushRecipeToMealie:
    ...
    async def test_push_recipe_with_image_success(self, sample_recipe_dict):
        ...
        with patch('recipe_agent.integrations.mealie_integration.httpx.AsyncClient') as mock_client_class:
            # Setup mock client
            ...
            assert post_call_args[1]['headers']['Authorization'] == f"Bearer {MEALIE_API_KEY}"
    ...
    async def test_slug_format_without_quotes(self, sample_recipe_dict):
        ...
        with patch('recipe_agent.integrations.mealie_integration.httpx.AsyncClient') as mock_client_class:
            ...
            mock_post_response = Mock()
            mock_post_response.json.return_value = "blumenkohlsteak-mit-goldener-grana-padano"
            mock_post_response.raise_for_status = Mock()
            mock_client.post = AsyncMock(return_value=mock_post_response)
...
class TestPushImageToMealie:
    ...
    async def test_upload_png_image_success(self):
        ...
        with patch('recipe_agent.integrations.mealie_integration.httpx.AsyncClient') as mock_client_class:
            ...
            assert put_call_args[1]['headers']['Authorization'] == f"Bearer {MEALIE_API_KEY}"
